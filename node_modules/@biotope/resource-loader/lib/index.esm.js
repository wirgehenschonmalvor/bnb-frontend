...
const isHtmlFilePath = (path) => /^(ht|f)tp(s?)\:\/\/[0-9a-zA-Z]([-.\w]*[0-9a-zA-Z])*(:(0-9)*)*(\/?)([a-zA-Z0-9\-\.\?\,\'\/\\\+&amp;%\$#_]*)?\.html([a-zA-Z0-9\-\.\?\!\,\'\/\\\+&amp;%\$#_]*)?$/gm.test(path);
...
const ensureNoHtmlFile = (path) => isHtmlFilePath(path) ? shift(path.split('/').reverse()).reverse().join('/') : path;
...
const ensureAbsolutePath = (location) => cond$1([
    ...
    [always, pipe(ensureNoLeadingSlash, concat$1(ensureTrailingSlash(ensureNoHtmlFile(location.href))))]
]);
